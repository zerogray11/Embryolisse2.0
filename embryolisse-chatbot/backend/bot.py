...
def recommend_products(context):
    ...
    if not products:
        return {
            "message": "I couldn't find any products matching your criteria. Would you like to try again?",
            "needs_followup": True
        }
    ...
    response = {
        ...
        "products": [],
        "followup_message": "Would you like help finding another product or building a skincare routine? (Or type 'no' if you're all set)",
        "needs_followup": True
    }
...
def recommend_routine(context):
    ...
    response = {
        ...
        "products": [],
        "followup_message": "Would you like help finding a specific product or building another routine? (Or type 'no' if you're all set)",
        "needs_followup": True
    }
    ...
    if not response["products"]:
        return {
            "message": "I couldn't create a suitable routine. Let's try again!",
            "needs_followup": True
        }
...
def chat():
    ...
    if user_message.lower() == 'no':
        farewell_response = {
            "response": "Thank you for shopping with Embryolisse! Have a wonderful day! 🌟",
            "context": {},  # Reset context
            "end_conversation": True
        }
        return jsonify(farewell_response), 200, response_headers

    # Reset context if user wants to start over
    if "start over" in user_message.lower() or "new search" in user_message.lower():
        context = {}
    ...
    if context['choice'] == 'product':
        if not context.get('category'):
            categories = ["Cleanser", "Toner", "Serum", "Exfoliant", "Moisturizer", "Mask", "Oil", "Eye Cream"]
            detected_category = next((category for category in categories if category.lower() in user_message.lower()), None)
            ...
            detected_skin_type = next((skin_type for skin_type in skin_types if skin_type in user_message.lower()), None)
            ...
            detected_concern = next((concern for concern in concerns if concern in user_message.lower()), None)
            ...
            detected_age_group = next((age_group for age_group in age_groups if age_group in user_message.lower()), None)

            if detected_age_group:
                context['age_group'] = detected_age_group
                response = recommend_products(context)
                # Reset context while keeping introduced=True for next interaction
                context = {'introduced': True}
                return jsonify({"response": response, "context": context}), 200, response_headers
            else:
                return jsonify({"response": "May I ask your age group? (e.g., 20s, 30s, 40s, 50s+)", "context": context}), 200, response_headers

    # Handle routine recommendation flow
